...
from models.user_db import User, UserStats
from models.user_schemas import UserCreate, UserLogin, Token, UserProfile
...
from models.user_db import UserRegionStat, Achievement
...
async def get_available_avatars(username: str, db: AsyncSession = Depends(get_db)):
    ...
    unlocked_regions = [ach.name.split(" de ")[-1].lower() for ach in user.achievements if "Campeón de" in ach.name]
    
    # Mapeo de nombre de medalla a key de REGIONAL_AVATARS si es diferente
    # Por ahora coinciden (Kanto, Johto, etc.)
...
async def update_avatar(data: AvatarUpdate, db: AsyncSession = Depends(get_db)):
    ...
    # Nota: Aquí se podría validar que el URL enviado esté en su lista de 'desbloqueados'
    # pero para agilidad permitiremos el cambio si es un URL válido.
...
async def submit_quiz_answer(data: AnswerSubmit, db: AsyncSession = Depends(get_db)):
    ...
    result = await db.execute(
        select(User)
        .options(selectinload(User.stats), selectinload(User.achievements), selectinload(User.region_stats))
    ...
    if data.is_correct and data.region:
        ...
        if region_stat.correct_answers == 10 and region_key in REGION_BADGES:
            ...
            ach_result = await db.execute(select(Achievement).filter(Achievement.name == badge_info["name"]))
            achievement = ach_result.scalars().first()
            if not achievement:
                achievement = Achievement(
                    name=badge_info["name"],
                    description=f"Adivinaste correctamente 10 Pokémon originarios de {region_key.capitalize()}.",
